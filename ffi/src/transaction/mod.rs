...
pub const TABLE_FEATURE_V2_CHECKPOINT: c_int = 0;
...
pub unsafe extern "C" fn add_table_features(
    ...
    let features = unsafe { collect_table_features(feature_codes, feature_count) };
    let custom_metadata =
...
/// Copy and validate integer feature codes before converting them to kernel features.
///
/// # Safety
///
/// For a nonzero count, `feature_codes` must point to `feature_count` readable integers.
unsafe fn collect_table_features(
    ...
    if feature_codes.is_null() {
        return Err(delta_kernel::Error::generic(
            "feature_codes must not be null when feature_count is nonzero",
        ));
    }

    unsafe { std::slice::from_raw_parts(feature_codes, feature_count) }
        .iter()
        .map(|code| match *code {
            TABLE_FEATURE_V2_CHECKPOINT => Ok(TableFeature::V2Checkpoint),
            other => Err(delta_kernel::Error::generic(format!(
                "invalid table feature code: {other}"
            ))),
        })
        .collect()
}
...
/// satisfy [`copy_commit_info_string`]'s safety contract.
unsafe fn collect_commit_info_entries(
    ...
    unsafe { std::slice::from_raw_parts(entries, entry_count) }
        .iter()
        .map(|entry| {
            let key = unsafe { copy_commit_info_string(&entry.key, "custom metadata key") }?;
            if !keys.insert(key.clone()) {
                return Err(delta_kernel::Error::generic(format!(
                    "duplicate custom metadata key: {key}"
                )));
            }
            let value = unsafe { copy_commit_info_string(&entry.value, "custom metadata value") }?;
            Ok((key, value))
        })
        .collect()
}
...
unsafe fn copy_commit_info_string(
    slice: &KernelStringSlice,
    field_name: &str,
) -> DeltaResult<String> {
...
mod tests {
    ...
    async fn test_add_table_features_commits_v2_with_custom_metadata(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let (_tmp_dir, table_url, store, engine, snapshot) =
            setup_add_table_features_test("feature_metadata").await?;
        let feature_codes = [TABLE_FEATURE_V2_CHECKPOINT];
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore)]
    async fn test_add_table_features_rejects_invalid_codes_and_pointer_count_pairs(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let (_tmp_dir, table_url, _store, engine, snapshot) =
            setup_add_table_features_test("feature_invalid_inputs").await?;
        let feature_codes = [TABLE_FEATURE_V2_CHECKPOINT];
        let invalid_feature_codes = [99];

        let invalid_code = unsafe {
            add_table_features(
                snapshot.shallow_copy(),
                engine.shallow_copy(),
                invalid_feature_codes.as_ptr(),
                invalid_feature_codes.len(),
                true,
                ptr::null(),
                0,
            )
        ...
        assert_add_table_features_error_contains(
            null_features,
            KernelError::GenericError,
            "feature_codes must not be null",
        );
    ...
    async fn test_add_table_features_post_commit_snapshot_handles_are_independent(
        ...
        let post_commit_snapshot = match unsafe {
            committed_transaction_post_commit_snapshot(&committed)
        } {
            OptionalValue::Some(snapshot) => snapshot,
            OptionalValue::None => panic!("feature commit should produce a post-commit snapshot"),
        };
